...
//			cmd_reply_a204(0,0xA2,0x1,0x11);
			...
			while(1)
			{
				...
				{
					while(FALSE!=XAxiDma_Busy(&AxiDma1, XAXIDMA_DMA_TO_DEVICE));
		...
		switch(type)
		{
			...
			case  0xD2:
				switch(id)
				{
					...
					case  0x07:
						if(result_d207==result)
						{
							ReplyStructA203Ack.AckResult = result_d207;
						}
						else
						{
							result_d207=result;
...
int run_cmd_d20A(StructMsg *pMsg)//多文件导出
{
	 int temp=0,ret=0,i=0,h=0,x=0,y=0;
	 uint32_t file_Num=0,Read_mode=0;
	 u16 unicode_u16=0;
	 WCHAR cmd_str_1[64]={0};
	 BYTE cmd_str_11[128]={0};
//	 BYTE Name_Table[128][10]={0};

	 file_Num = CW32(pMsg->MsgData[i+0],pMsg->MsgData[i+1],pMsg->MsgData[i+2],pMsg->MsgData[i+3]);
	 i=i+4;
	 Read_mode= CW32(pMsg->MsgData[i+0],pMsg->MsgData[i+1],pMsg->MsgData[i+2],pMsg->MsgData[i+3]);
	 i=i+4;
	 temp=i;

//	 char **Name_Table= (char ** ) wjq_malloc_m (file_Num*sizeof(cmd_str_11));
	 ...
	 for(x = 0; x < file_Num; x++)
	 {
		 ...
		 {
			 unicode_u16=(pMsg->MsgData[i++]|pMsg->MsgData[i++]<<8);
			 cmd_str_1[y] = ff_uni2oem(unicode_u16,FF_CODE_PAGE);
			 if(cmd_str_1[y]<=0x7E)
			 {
				 cmd_str_11[h++]=cmd_str_1[y];
			 }
			 else
			 {
				 cmd_str_11[h++]=(cmd_str_1[y]>>8);
				 cmd_str_11[h++]=cmd_str_1[y];
			 }
			 if (cmd_str_1[y] == '\0') break;
		}           // 文件路径
		xil_printf("%s %d  %s\r\n", __FUNCTION__, __LINE__,cmd_str_11);
//		memcpy(Name_Table[128][x],cmd_str_11,sizeof(cmd_str_11));
		i=temp+128;
		temp=i;
...
{
		unsigned short ret=0,sts,m=0,l=0,n=0;
		uint32_t Status=0,bw=0,i=0,len,io_thr=0;
	    char alphanumeric1[16];
	    char alphanumeric2[16];
	    char alphanumeric3[16];
		char cmd_str_11[10]={0};
...
void Internal_count(uint32_t *LEN,uint32_t *TIME,uint32_t *LastSize,uint32_t *Length)
{
	  ...
	  if(len>pack_size)
	  {
		  time=len/pack_size+1;
		  length=pack_size;
		  lastSize=((len%pack_size)/4+1)*4;
		  if((len%pack_size)==0)
		  {
			  time=len/pack_size;
			  length=pack_size;
